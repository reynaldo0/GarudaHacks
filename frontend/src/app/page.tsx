...
const features = [
  {
    ...
    desc: "AI estimates usable space inside each carriage using 4 ceiling fisheye cameras.",
    color: "from-blue-500 to-blue-600",
  },
  {
    icon: GitBranch,
    title: "Real-time Fusion Engine",
    desc: "4 camera occupancy grids fused into a single carriage map per frame.",
    color: "from-violet-500 to-violet-600",
  },
  {
    icon: BarChart3,
    title: "Redistribution AI",
    desc: "When congestion is detected, the system recommends the optimal target carriage.",
    color: "from-primary to-secondary",
  },
  {
    icon: Wrench,
    title: "Bogie Predictive Maintenance",
    desc: "CALES engine tracks cumulative asymmetric load exposure for health prediction.",
    color: "from-amber-500 to-orange-500",
  },
  {
    icon: DoorOpen,
    title: "Smart Door Automation",
    desc: "Door recommendations follow a safety chain with rule engine validation.",
    color: "from-emerald-500 to-emerald-600",
  },
  {
    icon: Bell,
    title: "Instant Alerts & Announcements",
    desc: "CRITICAL density triggers real-time WebSocket alerts and voice announcements.",
...
const DC: Record<string, { fill: string; border: string }> = {
  GREEN: {
    fill: "from-green-400/70 to-emerald-500/70",
    border: "border-green-300/60",
  },
  YELLOW: {
    fill: "from-amber-400/70 to-yellow-500/70",
...
export default function LandingPage() {
    ...
    (msg: { type: string; data?: Record<string, unknown> }) => {
      ...
      switch (msg.type) {
        ...
        case "occupancy_updated": {
          if (!msg.data) break;
          const carsList =
            (msg.data.cars as Array<Record<string, unknown>>) || [];
          if (Array.isArray(carsList)) {
            store.setCars(
              carsList.map((c) => ({
                ...
                densityIndicator:
                  (c.density_indicator as "GREEN" | "YELLOW" | "RED") ??
                  "GREEN",
                spatialOccupancyScore:
                  (c.spatial_occupancy_score as number) ?? 0,
        ...
        case "warning_updated": {
          ...
          const nw = {
            ...
            warningType:
              (w.warningType as string) ||
              (w.warning_type as string) ||
              "unknown",
          ...
          if (!store.warnings.some((x) => x.id === nw.id))
            store.setWarnings([...store.warnings, nw]);
          break;
        }
        case "system_health_updated": {
          ...
          store.setSystemHealth([
            {
              label: "Backend",
              status: h.backend === "running" ? "ok" : "error",
              value: String(h.backend ?? "unknown"),
              icon: "server",
            },
            {
              label: "AI Pipeline",
              status: h.ai === "loaded" ? "ok" : "warning",
              value: String(h.ai ?? "offline"),
              icon: "cpu",
            },
            {
              label: "Cameras",
              status: (h.cameras as number) > 0 ? "ok" : "warning",
              value: `${h.cameras ?? 0} active`,
              icon: "camera",
            },
            {
              label: "Uptime",
              status: "ok",
              value: `${Math.floor(((h.uptime as number) ?? 0) / 60)}m`,
              icon: "clock",
            },
            {
              label: "Warnings",
              status: (h.activeWarnings as number) > 0 ? "warning" : "ok",
              value: `${h.activeWarnings ?? 0} active`,
              icon: "alert",
            },
          ]);
        ...
        case "recommendation_changed": {
          if (!msg.data) break;
          const r =
            (msg.data.recommendation as Record<string, unknown>) || msg.data;
    ...
    async function fetchInitial() {
      try {
        ...
        if (occRes.status === "fulfilled" && occRes.value?.data?.cars) {
          setCars(
            occRes.value.data.cars.map((c: Record<string, unknown>) => ({
              ...
              densityIndicator:
                c.densityIndicator ?? c.density_indicator ?? "GREEN",
              spatialOccupancyScore:
                c.spatialOccupancyScore ?? c.spatial_occupancy_score ?? 0,
        ...
        if (warnRes.status === "fulfilled" && warnRes.value?.data?.warnings)
          setWarnings(warnRes.value.data.warnings);
        if (stateRes.status === "fulfilled" && stateRes.value?.data?.system)
          setUptime(stateRes.value.data.system.uptimeSeconds ?? 0);
      } catch {}
    }
    fetchInitial();
  }, [setCars, setWarnings]);
      ...
      ws.onclose = () => {
        ...
        if (retryRef.current < 5) {
          reconnectTimer.current = setTimeout(
            connect,
            Math.min(1000 * 2 ** retryRef.current, 15000),
          );
          retryRef.current++;
        }
      };
  ...
  const avgOcc =
    cars.length > 0
      ? cars.reduce((s, c) => s + c.occupancyRatio, 0) / cars.length
      : 0;
  ...
  const kpis = [
    ...
    {
      label: "RED Cars",
      value: String(redCars),
      icon: <AlertTriangle className="w-4 h-4" />,
      accent: redCars > 0 ? "text-red-600" : "text-green-600",
    },
    {
      label: "Warnings",
      value: String(critWarnings),
      icon: <Bell className="w-4 h-4" />,
      accent: critWarnings > 0 ? "text-amber-600" : "text-green-600",
    },
    {
      label: "Uptime",
      value: `${Math.floor(uptime / 60)}m`,
    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <p className="mt-6 text-base md:text-lg text-muted-foreground leading-relaxed max-w-xl">
                Real-time carriage occupancy estimation using ceiling fisheye
                cameras and AI segmentation. Measuring available usable space to
                support operational decisions.
              </p>
              <div className="flex flex-wrap gap-3 mt-8">
                <Link
                  href="/dashboard"
                  className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl bg-gradient-to-r from-primary to-secondary text-white font-medium shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 transition-all duration-200 hover:scale-[1.02]"
                >
                ...
                <a
                  href="#how-it-works"
                  className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl glass border border-border text-foreground font-medium hover:bg-muted/50 transition-all duration-200"
                >
                  How It Works
                </a>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-10">
                {kpis.map((s, i) => (
                  <div
                    key={i}
                    className="glass rounded-xl p-3.5 animate-fade-in"
                  ...
                  >
                    ...
                    <p className={clsx("text-xl font-bold", s.accent)}>
                      {s.value}
                    </p>
                  </div>
                ))}
              ...
              <div className="glass rounded-2xl p-5 border border-border/50 shadow-xl shadow-black/5">
                  ...
                  <div className="flex items-end gap-0 relative z-10">
                    <div className="flex-shrink-0 w-6">
                      <svg viewBox="0 0 24 40" className="w-full" fill="none">
                        <path
                          d="M22 3 C22 1.5, 20 0, 17 0 L7 0 C4 0, 2 1.5, 2 3 L2 37 C2 38.5, 4 40, 7 40 L17 40 C20 40, 22 38.5, 22 37 Z"
                          fill="hsl(var(--muted))"
                    ...
                    {Array.from({ length: 6 }, (_, i) => {
                      ...
                      return (
                        <div key={i} className="flex-1 mx-[1px]">
                          <div
                            className={clsx(
                              "relative rounded border-2 overflow-hidden transition-all duration-500",
                              dc.border,
                            )}
                          ...
                          >
                            <div
                              className={clsx(
                                "absolute bottom-0 left-0 right-0 bg-gradient-to-t rounded-b-sm",
                                dc.fill,
                              )}
                            ...
                            <div className="absolute top-[12%] left-[8%] right-[8%] flex gap-[5%] h-[30%]">
                              {[0, 1, 2].map((w) => (
                                <div
                                  key={w}
                                  className="flex-1 rounded-sm bg-white/15 border border-white/20"
                                />
                              ))}
                            </div>
                            <div className="absolute inset-0 flex items-center justify-center z-10">
                              <div className="text-center">
                                <span className="text-[10px] sm:text-xs font-bold text-foreground drop-shadow-sm">
                                  {i + 1}
                                </span>
                    ...
                    <div className="flex-shrink-0 w-3">
                      <svg viewBox="0 0 12 40" className="w-full" fill="none">
                        <path
                          d="M10 3 C10 1.5, 8 0, 6 0 L3 0 C1.5 0, 0 1.5, 0 3 L0 37 C0 38.5, 1.5 40, 3 40 L6 40 C8 40, 10 38.5, 10 37 Z"
                          fill="hsl(var(--muted))"
                          stroke="hsl(var(--border))"
                          strokeWidth="1"
                        />
                ...
                <div className="flex items-center gap-4 mt-3 text-[10px] text-muted-foreground">
                  <div className="flex items-center gap-1">
                    <div className="w-2 h-2 rounded-full bg-green-500" /> GREEN
                  </div>
                  <div className="flex items-center gap-1">
                    <div className="w-2 h-2 rounded-full bg-amber-500" /> YELLOW
                  </div>
                  <div className="flex items-center gap-1">
                    <div className="w-2 h-2 rounded-full bg-red-500" /> RED
                  </div>
                  <span className="ml-auto font-medium">
                    {cars.length} / 6 gerbong
                  </span>
                </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-muted-foreground mt-3 max-w-xl mx-auto">
              9 AI engines working in sequence to deliver real-time spatial
              intelligence.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f, i) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-11 h-11 rounded-xl bg-gradient-to-br ${f.color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
                  >
      ...
      <section id="how-it-works" className="py-20 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-muted-foreground mt-3 max-w-xl mx-auto">
              From camera capture to operational decision — 9 steps in under 5
              seconds.
            </p>
          </div>
          ...
          <div className="glass rounded-3xl p-10 md:p-14 text-center relative overflow-hidden">
            ...
            <div className="relative">
              ...
              <p className="text-muted-foreground max-w-lg mx-auto mb-8">
                Access the Operation Center for real-time spatial occupancy
                monitoring, AI-driven redistribution, and bogie predictive
                maintenance.
              </p>
              <Link
                href="/dashboard"
      ...
      <footer className="border-t border-border py-8 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-xs text-muted-foreground">
            AI-powered Railway Decision Intelligence Platform &middot; Garuda
            Hacks 2026
          </p>
        </div>
      </footer>
